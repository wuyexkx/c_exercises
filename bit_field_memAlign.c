...
// win下gcc中默认#pragma pack(8)，可以通过预编译命令#pragma pack(n)，n = 1,2,4,8,16来改变这一系数。
#pragma pack(4) // alignment must be a small power of two
...
// (1) 结构体第一个成员的偏移量（offset）为0，offset = 整数倍的min(有效对齐值, max size member of struct)
